...
router.get('/balance', async (req, res) => {
  try {
    ...
    const wallet = await Wallet.find({ AccountNumber: accountNumber }).select(
      'AccountBalance'
    );
    res.json({
      Message: `Your Current Balance is ${formatToCurrency(
        wallet[0].AccountBalance
      )}`
    });
  } catch (err) {
...
router.patch('/update', async (req, res) => {
  try {
    ...
    const response = await Wallet.findOneAndUpdate(filter, updateItem, {
      new: true
    });
    if (response === null) {
